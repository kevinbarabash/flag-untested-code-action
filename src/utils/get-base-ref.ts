...
const checkRef = (ref: string) =>
    spawnSync('git', ['rev-parse', ref, '--']).status === 0;

export const validateBaseRef = (baseRef: string): string | null => {
    ...
    if (GITHUB_BASE_REF) {
        ...
        for (let i = 1; i < 100; i++) {
            try {
                ...
                lines = lines.filter(
                    (line: string) => line !== `refs/heads/${headRef}`,
                );
...
export const cannedGithubErrorMessage = (): string => {
    const { GITHUB_BASE_REF } = process.env;
